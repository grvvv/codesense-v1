...
class ScanModel:
    ...
    def update_progress(cls, scan_id: str, **kwargs):
        """
        Update any subset of: total_files, files_scanned, findings, start_time, end_time, status
        Example: update_progress(scan_id, files_scanned=3, findings=7)
        """
        update_fields = {}
        for key in ["total_files", "files_scanned", "findings", "start_time", "end_time", "status"]:
            if key in kwargs:
    ...
    def find_by_project(cls, project_id: str):
        cursor = cls.collection.find({"project_id": ObjectId(project_id)})
